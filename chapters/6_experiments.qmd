...
This experiment aims to test maximum range and to obtain a transfer characteristic of the sensor. 
The purpose of the first UAV was to act as an observer and for the entire duration of the test remained at position $\begin{bmatrix}0 & 0 & 5\end{bmatrix}^T$. 
...
In each test one UAV acted as an observer and stayed stayed at position $\begin{bmatrix}0 & 0 & 5\end{bmatrix}^T$. The second UAV followed a circle of radius 10 m around the first UAV in car-like motion. The difference between the 4 experiments was relative angle respective to the velocity vector.
...
In *A sources of error in DW1000* [@qorvo:aps011] manufacturer mentions that there are two sources of error - clock drift and received signal level.
The implemented ranging technique (double-sided two-way ranging) compensates for the clock drift. Therefore only error source left is the received signal level. The more powerful the signal, the sooner it is timestamped, leading to a shorter distance reported. 
...
Further research should be made to correct errors dependent on the received signal level.
...
sort!(data, [:angle])
@. data.range_uwb += offset
plot!(data.angle, data.range_uwb - data.rtk_distance, legend=true, label="0")
data = DataFrame(CSV.File("data/data_90.csv"))
sort!(data, [:angle])
@. data.range_uwb += offset
plot!(data.angle, data.range_uwb - data.rtk_distance, legend=true, label="π/2")
data = DataFrame(CSV.File("data/data_180.csv"))
sort!(data, [:angle])
@. data.range_uwb += offset
plot!(data.angle, data.range_uwb - data.rtk_distance, legend=true, label="π")
data = DataFrame(CSV.File("data/data_270.csv"))
sort!(data, [:angle])
@. data.range_uwb += offset
plot!(data.angle, data.range_uwb - data.rtk_distance, legend=true, label="π3/4")
...
The algorithm was inspired by *UVDAR System for Visual Relative Localization With Application to Leader–Follower Formations of Multirotor UAVs* @8651535 and is represented in figure @fig-leader-follower.
...
The test for repeated two times. First the UWB and UVDAR fusion was kept for the whole time providing accurate position of the leader UAV. During the second test, the fusion process was halted midway, and only UVDAR measurements were utilized. 

The results depicted in Figure @fig-leader-follower-result demonstrate the advantages of fusing UWB and UVDAR technologies.
Due to the absence of precise distance measurements from UWB (Ultra-Wideband) technology, the follower UAV was unable to maintain track of the leader UAV.
