...
It's worth noting that the extended Kalman filter has some limitations. Since it uses linear approximations, it may not perform well for highly non-linear systems or when the linearization is inaccurate [@labbe2014]. In such cases, other techniques like the unscented Kalman filter or particle filters may be more appropriate.
